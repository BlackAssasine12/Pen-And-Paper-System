...
// Steigerungskosten
const levelUpCosts = [
  0, 2, 4, 6, 9, 12, 15, 18, 22, 26, 30, 34, 38, 43, 48, 53, 58, 64, 70, 76, 82,
];
  ...
  try {
    ...
    if (steigerungspunkteInput) {
      ...
      if (MagicSystem.getAdvancementPoints() !== punkteValue) {
        ...
        console.log(
          'Steigerungspunkte vom Charakter-Tab übernommen:',
          MagicSystem.getAdvancementPoints(),
        );
      }
    }
...
function levelUpMagic(index: number) {
  try {
    ...
    if (MagicSystem.getAdvancementPoints() < cost) {
      alert(
        `Nicht genügend Steigerungspunkte! Du brauchst ${cost} Punkte, um auf Level ${magic.level + 1} zu steigern.`,
      );
      return;
    }
...
function addPoints() {
  try {
    ...
    alert(
      'Es ist ein Fehler beim Hinzufügen von Steigerungspunkten aufgetreten. Siehe Konsole für Details.',
    );
  }
}
    ...
    tab.addEventListener('click', (event) => {
      ...
      if (target?.getAttribute('data-tab') === 'magie-tab') {
        ...
        setTimeout(() => {
          ...
          if (magieTabElement && !magieTabElement.querySelector('.vanilla-magic-system')) {
